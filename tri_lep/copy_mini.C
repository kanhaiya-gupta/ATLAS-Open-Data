	...
	if(trigE || trigM){
	  ...
	  if(goodlep_n==3 )
		...
		wcand = 3;
	      ...
	      float mtw=0;
	      if(wcand==1){ mtw = sqrt(2*Lepton_1.Pt()*MeT.Et()*(1-cos(Lepton_1.DeltaPhi(MeT))))/1000.;}
	      if(wcand==2){ mtw = sqrt(2*Lepton_2.Pt()*MeT.Et()*(1-cos(Lepton_2.DeltaPhi(MeT))))/1000.;}
	      if(wcand==3){ mtw = sqrt(2*Lepton_3.Pt()*MeT.Et()*(1-cos(Lepton_3.DeltaPhi(MeT))))/1000.;}
